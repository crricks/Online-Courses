...
# Recall that in each trial, the objective is to determine how many time-steps are on 
# average needed before a specified fraction of the room has been cleaned. Implement the following function:
...
# (This will allow us to easily adapt the simulation to run with different robot implementations, which you'll encounter in Problem 6.)
...
# You should also check your simulation's output for speeds other than 1.0. One way to do this is to take the above test cases, 
# change the speeds, and make sure the results are sensible.
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for t in range(num_trials):
        ...
        percCov = room.getNumCleanedTiles() / room.getNumTiles()
        steps = 0
        # Create list of robot instances
        allRobs =  [robot_type(room, speed) for rob in range(num_robots)]
        # Update positions and clean while percentage remains less than min_coverage
